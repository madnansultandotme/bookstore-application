...
<div class="mb-16">
    <div class="flex items-center justify-between mb-8">
        ...
        <a href="{{ route('books.index') }}" class="text-primary hover:text-indigo-700 font-medium transition flex items-center">
...
<div class="bg-white rounded-lg shadow-sm border border-gray-200 p-8 mb-16">
    ...
    <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
        @foreach($categories as $category)
            <a href="{{ route('books.index', ['category' => $category->id]) }}" class="group bg-gray-50 rounded-lg p-6 text-center hover:bg-primary hover:shadow-lg transition-all duration-300 border border-gray-200">
                <i class="fas fa-book text-3xl text-primary group-hover:text-white mb-3 group-hover:scale-110 transition-transform"></i>
                <h3 class="font-bold text-gray-900 group-hover:text-white transition">{{ $category->name }}</h3>
                <p class="text-sm text-gray-500 group-hover:text-white group-hover:opacity-90 mt-1 transition">{{ $category->books_count }} books</p>
            </a>
        @endforeach
    </div>
</div>
...
<div class="bg-primary rounded-lg shadow-lg overflow-hidden mb-16">
    <div class="px-6 py-12 sm:px-12 lg:flex lg:items-center lg:justify-between">
        <div class="lg:w-2/3">
            ...
            <a href="{{ route('books.index') }}" class="inline-flex items-center justify-center px-6 py-3 border-2 border-white text-base font-medium rounded-md text-white hover:bg-white hover:text-primary transition">
                Shop Now
                <i class="fas fa-arrow-right ml-2"></i>
            </a>
        </div>
